...
BoatPath::BoatPath(TerrainMap &map, std::string name, Point start, Point finish) : Path(map, std::move(name), start, finish) {
...
std::vector<Point> BoatPath::getNeighbours(Point current) {
    ...
    for (int j = -1; j <= 1; ++j) {
        for (int i = -1; i <= 1; ++i) {
            ...
            if (map.validCoords(Point(current.x + i, current.y + j))) {
                ...
                if (map.alt(candidate) < 0 || candidate == finish) {
